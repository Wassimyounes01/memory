'use strict';
/**
 * mneme.cjs — a tiny, dependency-free memory layer for LLM agents.
 *
 ...
 *                 — if you set MNEME_RERANKER to an LLM adapter — hands the candidates to that
 *                 model to rerank and answer. No reranker? It degrades to keyword-only and
 *                 never throws.
 *
 * Shares the SAME files as python/mneme.py (stdlib-only twin) — one source of truth.
 *
 * API:
 *   const M = require('./mneme.cjs');
 ...
 *
 * CLI: node lib/mneme.cjs health | remember "..." | recall "query" [k]
...
const DIR = process.env.MNEME_DATA_DIR
  ? path.resolve(process.env.MNEME_DATA_DIR)
  : path.join(ROOT, 'data');
...
/**
 * Load the optional LLM reranker. Set env MNEME_RERANKER to a module path that exports
...
function loadReranker() {
  const p = process.env.MNEME_RERANKER;
  if (!p) return null;
  return safe(() => {
...
function health() {
  return {
    ok: true,
    engine: process.env.MNEME_RERANKER ? 'files+reranker' : 'files',
    reranker: process.env.MNEME_RERANKER || null,
    data_dir: DIR,
